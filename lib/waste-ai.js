...
const TRAINED_LABELS = (process.env.WASTE_MODEL_LABELS || 'paper,plastic,bottle')
  .split(',')
...
export function createVerificationDecision(prediction, expectedLabel) {
  ...
  if (!TRAINED_LABELS.includes(prediction.label)) {
    return {
      normalizedExpected,
      labelMatches: false,
      decision: {
        status: 'rejected',
        message: `Item not recognised as ${TRAINED_LABELS.join(', ')}. No points awarded.`,
      },
    };
  }
  ...
  const decision = labelMatches
    ...
    : {
        status: 'rejected',
        message: `Model detected ${prediction.label}, but this quest expects ${normalizedExpected}. Place the correct item and try again.`,
      };
